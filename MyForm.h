...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->тёмнаяToolStripMenuItem->Size = System::Drawing::Size(215, 22);
			this->тёмнаяToolStripMenuItem->Text = L"Информационная сводка";
			...
			this->label8->Text = L"Целая часть:";
			// 
			// label9
			// 
			...
			this->label10->Text = L"Знаменатель:";
			// 
			// MyForm
			// 
...
#pragma endregion
	...
	private: System::Void label6_Click(System::Object^ sender, System::EventArgs^ e) {
		   ...
		   //проверка на ввод других символов кроме цифр и минуса
		   bool flag = true;
		   bool Protect() {
			   ...
			   for (int i = 0; i < textBox1->Text->Length; i++) {
				   ...
				   else { MessageBox::Show("Ошибка ввода!", "Ошибка!"); flag = false; return false; }
			   }
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		//основа программы-совершает действия между дробями и выводит результат, во всех случаях деления на ноль выводит соответствующее сообщение об ошибке
		if (Protect()) {
			...
			if (comboBox1->SelectedItem == "Сложение") {
				...
				if (x1 == 0 && x2 == 0) {
					if (Num2 == 0 || Num4 == 0) {
						MessageBox::Show("На ноль делить нельзя!", "Ошибка!"); return;
					}
					else {
						Num1 = Num1 * Num4;
						Num3 = Num3 * Num2;
				...
				if (x1 == 0 && x2 == 0) {
					...
					else { MessageBox::Show("На ноль делить нельзя!", "Ошибка!"); return; }
				}
				else if (x1 != 0 && x2 != 0) {
					if (Num2 != 0 && Num4 != 0) {
						if (x1 < 0 && x2 > 0) {
		...
		else { MessageBox::Show("Заполните поля!", "Ошибка!"); }	//сообщение о пустоте полей
	}
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
		//работа кнопки очистки
		textBox1->Text = L"";
		textBox2->Text = L"";
		textBox3->Text = L"";
		textBox4->Text = L"";
	...
	private: System::Void тёмнаяToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
		MessageBox::Show("Творческая работа-калькулятор\n" "Бородин А.А.\n" "ИВТ - 21 - 2б\n" "ПНИПУ\n ", "Информационная сводка");	//текст в кнопке информации о творческой работе
	}
	private: System::Void оПрограммеToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
		MessageBox::Show("Калькулятор обычных дробей с целыми частями", "О программе");	//текст в кнопке о калькуляторе
	}
	private: System::Void MyForm_FormClosing(System::Object^ sender, System::Windows::Forms::FormClosingEventArgs^ e) {
		//реализация меню выхода из программы
		if (MessageBox::Show("Выход", "Выход из программы", MessageBoxButtons::YesNo, MessageBoxIcon::Question) == System::Windows::Forms::DialogResult::No) {
			e->Cancel = true;
		}
		else { e->Cancel = false; }
	}
